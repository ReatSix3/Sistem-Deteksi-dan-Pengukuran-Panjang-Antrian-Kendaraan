...
while cap.isOpened():
    ...
    for i in range(len(tracked_detections)):
        ...
        if len(history) > 1:
            distance_px = np.linalg.norm(np.array([history[-1].x, history[-1].y]) -
                                         np.array([history[-2].x, history[-2].y]))
    ...
    if current_frame_data:
        labels = [f"ID:{d['track_id']} S:{d['speed_kmh']:.1f}" for d in current_frame_data]
